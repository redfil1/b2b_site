...
каталог-витрина: клиент выбирает товар → связывается с менеджером напрямую (переход по
простой ссылке на `/contacts`, без форм и без `tel:`/`mailto:`) → менеджер вручную готовит
КП и согласовывает заказ у поставщика. Личный кабинет и авторизация на этом этапе не
реализуются.
...
- Не использовать `/cart` и `/request` — единственный призыв к действию это ссылка на
  `/contacts`, без форм сбора данных.
