...
на сайте нет (юридическое решение от 2026-08-31, 152-ФЗ). Два равноправных пути обращения к
менеджеру: (а) с карточки товара — простая ссылка на `/contacts?product=<slug>`, без форм и
без `tel:`/`mailto:`; (б) корзина `/cart` (решено 2026-09-06, `Architecture.md`, раздел 2.3) —
несколько товаров сразу, состояние только в `localStorage` браузера, без сервера/БД; сводку
клиент передаёт менеджеру сам — через email (Gmail/Outlook/`mailto:`, `Frontend.md`, раздел
7.4) или копированием текста. Ни один из путей не собирает персональные данные на сервере сайта.
...
- `/request` не существует и не появляется. `/cart` существует (решено 2026-09-06) строго в
  своих границах — client-side, `localStorage`, без сервера/БД, без API-роута; не расширять
  эти границы (например, не заводить под неё бэкенд/БД) без нового явного решения
  пользователя. Основной призыв к действию вне `/cart`/`/contacts` — по-прежнему только
  ссылка на `/contacts`.
