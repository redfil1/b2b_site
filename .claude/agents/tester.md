...
без личного кабинета и без оплаты на сайте. "Живых" сетевых сценариев нет — два пути
обращения к менеджеру: простая ссылка на `/contacts?product=<slug>` с карточки товара, и
корзина `/cart` (несколько товаров сразу, состояние только в `localStorage` браузера, без
сервера/БД — `Architecture.md`, раздел 2.3). Тестового фреймворка (Jest/Vitest/Playwright и
т.п.) в проекте не установлено, отдельного `npm run test` нет.
...
- Основной призыв к действию (шапка, карточка товара) — ссылка на `/contacts`, без
  `tel:`/`mailto:` и без форм сбора данных вне `/cart`/`/contacts`; `/request` не существует.
  `/cart` существует и не должен обращаться к серверу/БД или собирать персональные данные —
  состояние только в `localStorage`.

## Чего не делать
- Не выдумывать поля/контракты, которых нет в `docs/*.md`.
- Не проверять `/request` — этого роута не существует. `/cart` — проверять как обычный
  роут (он статически пререндерится, `next build` это покроет), но не выдумывать для него
  серверные сценарии — состояние там только в `localStorage`, без сервера/БД.
