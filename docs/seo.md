...
`Category.image`; при появлении новых мест — то же правило.

**Временно не действует (аудит 2026-09-05).** `Category.image` сейчас не заполнен ни у
одной категории — плитки на `/catalog` и на главной вместо фото показывают декоративную
line-иконку (`CategoryIcon`, `Frontend.md`, раздел 4.3.1), помеченную `aria-hidden`, а не
`alt` (название категории и так есть текстом рядом с плиткой, скринридеру нечего терять).
Правило снова вступает в силу, когда `Category.image` заполнится реальными фото и плитка
вернётся на `next/image`.
...
### 9.1 Та же мера распространена на черновые статичные страницы (самокритичный аудит, 2026-09-06)

Та же причина, что у `/catalog/*` выше («не индексировать, пока контент не финальный»),
в равной мере относится к страницам, которые прямо показывают посетителю баннер
`DraftNotice` («Текст черновой, требует проверки перед публикацией») — раньше это
почему-то не было замечено и `noindex` на них не ставился, хотя условие снятия то же
самое (заменить черновой текст на финальный). Получили `robots: { index: false }`:

- `app/company/page.tsx`, `app/services/page.tsx`, `app/delivery/page.tsx`,
  `app/contacts/page.tsx` — статический `export const metadata`;
- `app/legal/[doc]/page.tsx` — `generateMetadata`, для обеих известных страниц
  (`privacy`, `offer`); неизвестный `doc` — отдельный случай, раздел 9.2.

Убирается тем же способом, что и у `/catalog/*` — когда конкретная страница получает
финальный текст и `DraftNotice` с неё убирается.

### 9.2 `/legal/[doc]` перестал принимать произвольный slug (самокритичный аудит, 2026-09-06)

Раньше `app/legal/[doc]/page.tsx` не проверял `doc` вообще — любой несуществующий путь
(`/legal/что-угодно`) отдавал HTTP 200 с общим текстом-заглушкой вместо 404 («мягкий
404»). Теперь `doc`, которого нет в `KNOWN_LEGAL_DOC_TITLES`, вызывает `notFound()` в
самой странице; `generateMetadata` для этого случая отдаёт `{ title: "Документ не
найден", robots: { index: false } }` — тем же паттерном, что уже используется для
несуществующей категории/товара каталога (`generateMetadata` в
